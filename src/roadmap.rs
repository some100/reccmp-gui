//! Using roadmap for better disassembly analysis.
//!
//! This is a probably unintended use case of roadmap, but we use it for two
//! main reasons:
//!
//! 1. It allows us to resolve symbols independently of reccmp-reccmp, meaning that we can use our own
//!    disassembler rather than relying on reccmp's.
//! 2. It gives us the metadata of the reccmp csvs for free, so it saves us from having to parse those as well
...
impl RoadmapRow {
    ...
    pub fn size(&self) -> Option<u64> {
        let size = self.size.as_deref()?.trim();
        if size.is_empty() {
            return None;
        }
