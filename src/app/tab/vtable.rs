...
impl VtableTab {
    ...
    fn parse_vtable_line(asm: &str) -> (Option<Address>, Option<Address>, String) {
            ...
            && let Some((addrs_part, name_part)) = rest.split_once(')')
        {
            ...
            if let Some((orig_str, recomp_str)) = addrs_part.split_once('/') {
                return (orig_str.parse().ok(), recomp_str.parse().ok(), name);
            }

            return (addrs_part.parse().ok(), None, name);
        }

        (None, None, trimmed.to_string())
    }

    fn render_table(&self, ui: &mut egui::Ui) -> Option<String> {
        ...
        ui.add(
            VirtualTable::new(
                ...
                |ui, row_idx, row_height| {
                    ...
                    if let Some(target) = Self::render_row(ui, row, row_height) {
                        target_to_open = Some(target);
                    }
                },
            )
    ...
    fn render_row(ui: &mut egui::Ui, row: &VtableRow, row_height: f32) -> Option<String> {
        let mut target_to_open = None;
        ...
        ui.vertical(|ui| {
            ...
            if self.rows.is_empty() {
                if self.matching >= 1.0 {
                    ui.label("Vtable is 100% matching.");
                } else {
                    ui.label("Waiting for report...");
                }
                return;
            }
            ...
            if let Some(name) = self.render_table(ui) {
