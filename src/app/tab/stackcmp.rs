...
impl StackcmpTab {
    ...
    pub fn update_report(&mut self, report: StackcmpReport) {
        self.stats = StackcmpStats::new(&report.ordered_by_orig);
        self.report = report;
        self.has_run = true;
        self.error = false;
        self.update_filter();
...
impl TabView for StackcmpTab {
    fn render(&mut self, ui: &mut egui::Ui) -> Option<UiAction> {
        ui.vertical(|ui| {
            if self.error {
                ui.label("reccmp-stackcmp failed. See logs for details.");
                return;
            }
            if self.report.ordered_by_orig.is_empty() && self.report.ordered_by_recomp.is_empty() {
                if self.has_run {
                    ui.label("Function has no stack variables.");
                } else {
                    ui.horizontal(|ui| {
                        ui.label("Waiting for stackcmp...");
                        ui.spinner();
                    });
                }
                return;
