...
impl RoadmapFilter {
    fn matches(&self, row: &RoadmapRow) -> bool {
...
impl RoadmapTab {
    ...
    fn render_toolbar(&mut self, ui: &mut egui::Ui) -> bool {
        ...
        ui.horizontal_wrapped(|ui| {
            ...
            egui::ComboBox::from_id_salt("roadmap_module_filter")
                .selected_text(&self.filter.module)
                .show_ui(ui, |ui| {
    ...
    fn render_row(&self, ui: &mut egui::Ui, row_height: f32, idx: usize) -> Option<UiAction> {
        ...
        let mut func_to_open = None;
        let mut vtable_to_open = None;
        ui.add(
            TableRow::new(&TABLE_COLUMNS, row_height)
                ...
                .cell(RichText::new(&row.module).color(Color32::GRAY))
                .trailing_ui(&mut |ui| {
                    ui.horizontal(|ui| match row.row_type() {
                        RoadmapRowType::Function => {
                            let label = ui.link(&row.name);
                            if label.clicked() {
                                func_to_open = Some(row.name.clone());
                            }
                        }
                        RoadmapRowType::Vtable => {
                            let label = ui.link(&row.name);
                            if label.clicked() {
                                vtable_to_open = Some(row.name.clone());
                            }
                        }
                        _ => {
                            ui.label(&row.name);
        ...
        if let Some(name) = func_to_open {
            return Some(UiAction::OpenFunctionByName(name));
        } else if let Some(name) = vtable_to_open {
            return Some(UiAction::OpenVtableByName(name));
        }
